...
| **親稿** | `drafts/series/論文0_忘却の忘却_草稿.md` v0.16 |
| **作成日** | 2026-04-29 |
...
候補は Boolean square である。

```text
[0] < [A] < [A∨B] > [B] > [0]
```

ただし nerve の 1-cycle として非自明に残るには、2-simplex で埋まらない形に制御する必要がある。単なる菱形は poset の上位元・下位元により contractible になりうるため、ここは最初の危険点である。
...
| C2-b | その cycle が `N(L_{\le a})` の boundary として潰れない |
...
[主観] R2 の有限段階 counting は最初の足場として使えるが、それだけでは弱い。最も筋がよいのは、R2 で候補 cycle を絞り、C-3 で「証明像から同じ class を代表できない」ことを別に示す二段構えである。R1 は強力だが、対角化の別名になりやすいので、本 seed では第一選択にしない。
...
1. R2 counting route で、`L_{\le a}` の有限 Boolean subalgebra と `P_{\le a,b}` の生成自由度を比較する toy model を作る。
2. Boolean square が nerve で潰れる条件を調べ、cycle 実現に poset nerve で足りるか、cover nerve が必要かを判定する。
3. C-3 の「同じ homology class を代表できない」条件を、辺欠損・経路欠損・relative chain のどれで書くか決める。
