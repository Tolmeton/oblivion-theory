...
**版**: v0.12 (2026-05-01, **3 誤配位 3 番目を multiverse predictive underdetermination 批判 (Ellis & Silk 2014) に置換** — 旧「超ひも landscape (Hossenfelder 2018)」が「批判=誤配位 → 暗黙に超ひも擁護」と読める含意を持ち、エッセイ『反証可能性は死んだ.md』§8 / §8.6 の「超ひも理論自体が U_self / 全予測の野心で死亡」という立場と矛盾していた問題を解消。本稿は批判の category error の形式診断に scope を限定 (§7.3 末尾に射程明示注追加)、超ひも理論自体の structural critique は独立論文 (Paper 2、エッセイの formal 化) として後送。置換対象は要旨 / §1 冒頭 / §1.2 C5 / §1.4 表 / §2.1 / §7 冒頭 / §7.3 全節 rewrite / §7.4 表 / §7.5 表 / §8 / §M3 / §M5 の 17 箇所 (L441 VFE landscape は物理用語として保留)。§7.3 SOURCE は反証可能性は死んだ.md cross-reference を削除し Ellis & Silk 2014 *Nature* + §1.2 C4 + §6.6 内独立導出に anchor 化)
**前版**: v0.11 (2026-05-01, **要旨を構造保存定理整合形に精密化** — 第1段冒頭に「世界をより忠実に表現する関手 $F$ の漸近的探求」をメタ目的として主題化。構造保存定理 (本稿 §6.1; Paper VII §6.1 系 6.1.5) を要旨で明示参照し、equivalence 到達可能性を構造レベルに局在化 ($\eta|_{\text{構造}} \to \text{iso}$)、値レベルの $\text{Ker}(\eta|_{\text{値}}) > 0$ 永続性を明示。第2段 $\eta_{\text{unit}}$ 説明と C2 帰結節を構造-値分離形に精密化。Tolmetes 直接介入 + ボヤキ /u+ 深化結果反映 (痕跡から操作の生成それ自体は正当 / 取り違えはカテゴリーエラー / 反証可能性は値レベル真理₁ の局所評価器 / オイラー恒等式に反証可能性を要求するのと同型のドメインエラー))
**前々版**: v0.10 (2026-05-01, **要旨追加 + §1 構造再編 + §6.6 値問題と構造問題の反転型錯誤 追加** — 査読論文形式 (要旨→導入→本文) への移行。要旨を §1 直前に挿入 (4 段 + Keywords、5C2=10 ペア横断 + Bogen-Woodward + GAFT/SAFT + 達成度 86-91.5% honest 較正 + 予測主義一般への射程再固定)。「## 序」→「## §1 痕跡から操作へ — 三誤配位の再定式化」に昇格、序内容を §1 冒頭に保存しつつ「真理₀ (構造真理)」 Strategy 1.5 gloss 追加、「過ち = 痕跡から操作の生成ではなく、痕跡 (値) を構造証拠と同一視する取り違え」の精密化を §1 冒頭に統合。旧 §0 範囲限定を §1.3 射程の切断に吸収。§6.6 で値問題/構造問題の対称型錯誤を明示し、Popper 型反証 vs 構成的反例 ($S_3$ / Weierstrass) の区別と「構造を批判したいなら構造的反例を求めよ」の prescription を立て、本稿論証核への自己適用 (経験的予測の外れは個別事例・補助接続を傷つけうるが論証核への反証にはならない) で締める)
**前々々版**: v0.9 (2026-04-26, **Round 5 G-λ NRFT 完全形式化部分達成** — §8.4.3.1 で vdG-O 2020 (arxiv 2004.10482) + Yanofsky 2003 (arxiv math/0305282) + §5.5 で Goldfeld-Polyanskiy 2020 (arxiv 2011.06208) Corollary 1 完全 PDF Read 経由で Joyal AU の 4 公理 + $U_0$ 構成 + Lan ⊣ Syn equivalence + Cantor categorical Gödel 第二 + Yanofsky restatement 経由 Lawvere FP statement + Gaussian IB scalar closed form 接地。**§8.4.1.1** で **G-θ'-1 部分着手**、**§8.4.3.2** で **G-θ'-2 ~57% 達成**、**§8.4.3.3** で **HA-1 ~30% 部分達成 + Reduction 命題 [仮説 70%]** (HA-1 → G-θ'-1 (2)(4) + topos 公理への構造的帰着を発見)、**§5.5** で **G-ε 60% → 75-80%**。**G-θ'-3 解消** + **G-θ'-4 ~1.7/3 達成** + **G-θ'-1 部分着手** + **G-θ'-2 ~57% 達成** + **G-ε 75-80% 達成**。**honest 訂正**: 旧「Lawvere FP への reduction」は不正確、Lawvere-like FP (vdG-O Lemma 6.12) は Löb 用、Gödel 第二は Cantor categorical 直接使用。**Reduction 命題の含意**: 道 C 達成度の天井は G-θ'-1 (2)(4) 完全解消で律速される。道 C 達成度 60-70% → **86-91.5%** (honest)、C4 主張水準 仮説 65% → **仮説 70%**)
**前々々々版**: v0.8 (2026-04-26, Round 6 G-η 全 10 ペア natural transformation 骨格 追加 — §3.6.1 で 5 分野 5C2=10 ペア全てに対し核となる対応 + natural transformation component (iso / non-iso) + naturality 構造 + 残ギャップを骨格として固定)
**前々々々々版**: v0.7 (2026-04-26, Round 6 軽量着手 — G-ι 部分昇格 (Mayama et al. 2025 arxiv 2510.04084 完全 PDF 取得、Φ ↔ Bayesian surprise 強相関 ρ=0.879 の経験的橋渡しを §6.1 に統合) + G-ε 部分昇格)
**前々々々々々版**: v0.6 (2026-04-25, §8.4 Predictions Descend Theorem の形式証明試行 追加 — 達成度 60-70% で honest 較正)
...
科学とは、世界をより忠実に表現する関手 $F$ を漸近的に探求する営みである。本稿の構造保存定理 (§6.1; Paper VII §6.1 接続) により、$F$ は構造を保存し値を忘却する: $F$ は構造レベルでは漸近的に圏論的同値 (equivalence) に到達可能だが ($\eta|_{\text{構造}} \to \text{iso}$、系 6.1.5)、値レベルでは原理的に到達不能であり ($\eta|_{\text{値}} \neq \text{iso}$)、核 $\text{Ker}(\eta|_{\text{値}}) > 0$ が常に残る。20 世紀の科学哲学はこの構造-値の非対称性を直視せず、**値レベル (真理₁) の評価器 (反証可能性) を、構造レベル (真理₀) の評価器として読み替える**カテゴリーエラーを 70 年にわたって反復してきた。Popper の反証可能性、Mangalam の予測至上主義、multiverse への "predictive underdetermination" 批判 — いずれも、経験的予測 (値レベル) の整合を理論が世界の構造を捉えた指標として要求し、その不在を構造の不在として読む。

本稿は理解を **二つの関手の組** $L \dashv R$ として再定式化する。$L$ (還元関手, 上昇方向) は具体値を構造へ抽象化し、$R$ (回復関手, 下降方向) は構造を有限理論経由で具体値へ降ろす。両者は随伴対であるが、合成の単位 $\eta_{\text{unit}}: \text{Id} \Rightarrow R \circ L$ は **構造に対しては漸近的に同型に近づきうるが ($\eta|_{\text{構造}} \to \text{iso}$、構造保存定理 §6.1 系 6.1.5)、値に対しては原理的に同型にならない ($\eta|_{\text{値}} \neq \text{iso}$)**。したがって核 $\text{Ker}(\eta_{\text{unit}}|_{\text{値}})$ が常に残り、予測は原理的に消滅しない。本稿はさらに構造を **真理₀ (構造真理: 値・座標・観測手続きに先立つ普遍構造)** と **真理₁ (経験真理: 真理₀ を有限理論として世界に接続したもの)** に二層分解し、予測も **予測₀ (構造予測: 形の予告)** と **予測₁ (値予測: 初期条件・境界条件を入れて出す具体値)** に分ける。これに対応して補完も **補完₀ (構造補完: 真理₀ レベルでの予測誤差吸収)** と **補完₁ (経験補完: 真理₁ レベルでの予測誤差最小化のための補完操作)** に分かれる。

Predictions Descend とは、予測₁ が真理₀ から $R$ で生成される痕跡にすぎず、予測₁ の産出は理論が真理₀ を捉えた証ではないという主張である。$R$ による痕跡生成それ自体は科学の正当な営みであり、過ちではない。**過ちは、$R$ の像 (予測₁) を $L$ が運ぶべき構造証拠 (真理₀) と同一視し、両者を一つの評価軸に押し込めることにある**。反証可能性は値レベル (真理₁) の評価器として正当だが、それを構造レベル (真理₀) に押し付けると、オイラー恒等式に反証可能性を要求するのと同型のカテゴリーエラーが生じる。個別具体の値を求める探求と、普遍一般の構造を求める探求は、質的に異なる評価器を要求する。三誤配位はいずれも、この **質の混同** の異なる現れであり、本稿はこれを **関手の方向逆転誤読** ($R$ 方向の痕跡を $L$ 方向の構造証拠と等置する誤り) として定式化し、§7 で C5 (3 誤配位は単一構造的誤測定) として診断する。

この $L \dashv R$ 構造は単一分野の比喩ではなく、5 分野を横断する同型として現れる。情報幾何の Fisher 計量の非等方性、ゲージ理論の接続と曲率、統計力学の自由エネルギー最小化、数論の Peano successor と値の分離、自由エネルギー原理 (FEP) の予測誤差と補完。各分野で $L$ と $R$ は異なる具体を取るが、随伴の骨格と $\eta_{\text{unit}}$ 非同型は不変であり、5C2 = 10 ペア全てに自然変換成分が走る (§3)。Bogen-Woodward 三層 (data / phenomena / theory) との関手的読み替えと、GAFT / SAFT による $L \dashv R$ の存在条件 (§2) を経て、Predictions Descend Theorem は Joyal arithmetic universe の 4 公理と Goldfeld-Polyanskiy 2020 Gaussian Information Bottleneck を経由して達成度 86-91.5% (honest 較正) で部分達成される (§8.4)。射程は、予測を理解または真理₀の直接指標とみなす判定基準一般に及ぶ (§1.3)。

帰結として本稿は 5 核主張 (§1.2) を立てる。**C1** 科学における「理解」は随伴対 $L \dashv R$ の内在化として定義される関手的操作である。**C2** $\eta_{\text{unit}}$ は構造に対しては漸近的同型 ($\eta|_{\text{構造}} \to \text{iso}$) だが値に対しては原理的非同型 ($\eta|_{\text{値}} \neq \text{iso}$) であり、$\text{Ker}(\eta_{\text{unit}}|_{\text{値}}) > 0$ は全理論共通の原理的制約である (構造保存定理から帰結)。**C3** 補完₁ は $\text{Ker}(\eta_{\text{unit}})$ と随伴を介して結びつき、理解の深化は補完₁ 依存を単調減少させる。**C4** 予測₁ の産出は真理₀ の指標ではなく、真理₀ から下降関手 $R$ で生成される痕跡である (**Predictions Descend**)。**C5** Popper の反証可能性、Mangalam の予測至上主義、multiverse predictive underdetermination は C4 の同一系であり、3 大誤配位は単一の構造的誤測定 (上述の関手の方向逆転誤読) から派生する。

**Keywords**: 随伴関手 (adjoint functor), 自然変換 (natural transformation), Bogen-Woodward 三層, 反証可能性 (falsifiability), 自由エネルギー原理 (free energy principle), 情報幾何 (information geometry), ゲージ理論 (gauge theory), 統計力学 (statistical mechanics)
...
操作とは「**理解**」であり、痕跡とは「**予測**」である。Popper の反証可能性、Mangalam の予測至上主義、multiverse への "predictive underdetermination" 批判 — これら 70 年にわたる三つの誤配位は、すべて **値の理解と構造の理解を同一の評価軸で測る**という同じ過ちから派生する。痕跡から操作を生成すること自体は科学の正当な営みである。過ちは、痕跡 (値) を構造証拠と同一視することにある。
...
予測₁ (経験的予測) の産出は、理論が真理₀ (構造真理) を捉えた証ではない。それは真理₀ から下降関手 $R$ で生成される **痕跡** にすぎない (Predictions Descend)。本稿はこの誤読に対して、$L$ と $R$ を独立に並置し、両者の随伴を明示する (真理₀ / 真理₁ の定義は §2 で立てる)。
...
- **C5** [構造的類似, 仮説 55%]: ポパーの反証可能性 / Mangalam の予測至上主義 / multiverse predictive underdetermination は C4 の同一系。3 大誤配位は単一の構造的誤測定 (関手の方向逆転誤読) から派生する

### §1.3 射程の切断 (Scope Severance)

本稿の射程は次の 3 点で明示的に切断される。

1. **判定基準一般**: 本稿の射程は、予測を理解または真理₀の直接指標とみなす判定基準一般に及ぶ。予測₁は $R$ の像であり、そこから $L \dashv R$ の内在化や $T_0(T)$ を一意に復元する逆操作は構成できない。現世代 AI 評価の benchmark culture は、この構造の一例であり、射程の根拠ではない。
...
本章タイトル「痕跡から操作へ」の論理面に沿って、以降は次の順で進む。**§2 で随伴対 $L \dashv R$ を数学的に立て** (操作の最小骨格)、**§3 でそれが 5 分野横断の同型として現れることを示し** (痕跡が分野を超えて同じ構造から生成される証拠)、**§4 (Yoneda) と §5 (Information Bottleneck) で評価器の質的差異を支える定理基盤を固める** (痕跡レベルの評価と構造レベルの評価が同一軸に乗らない数学的根拠)。**§6 で射程と制約を切り、§7 で 3 誤配位を corollary として診断し** (痕跡を構造証拠と取り違える誤りの 3 つの現れ)、**§8 で Predictions Descend Theorem を不可能性定理として閉じる** (操作を痕跡から逆算する完全 retraction の不在)。
...
| §6 | 制約節 (3 立場の ontological commitment スペクトル / IIT との同型 / 予測主義一般の射程明示 / 残余問題) |
| §7 | corollary (Popper / Mangalam / multiverse predictive underdetermination) |
| §8 | 結語 (Predictions Descend の単一図式 + §8.4 形式証明試行で達成度 86-91.5% honest 較正) |
...
**§8.4 形式証明試行への anchor 予告**: 上記延長 4 (NRFT 骨格) は §8.4 で **Predictions Descend Theorem の形式証明試行** として展開される。Joyal arithmetic universe の 4 公理 (vdG-O 2020) + Yanofsky 2003 経由 Cantor categorical 対角化補題 + Goldfeld-Polyanskiy 2020 Gaussian Information Bottleneck の三本柱で形式化道筋を組み、Round 5 honest 較正で **達成度 86-91.5%** に固定する (詳細は §8.4.4)。本稿の Bogen-Woodward 延長は単なる記述的精密化ではなく、**Gödel 級不可能定理の関手論的実装** として閉じることを目指す。
...
3. **科学は真理₀ へ近づくために真理₁ を鍛える営みであり、工学は真理₁ を制御可能性へ翻訳する営みである** (§7 で Popper / Mangalam / multiverse predictive underdetermination の 3 誤配位がこの 1-3 のどこを取り違えているかを特定する)。
...
§1.3 で切った通り、本稿の形式的射程は、予測₁を理解または真理₀の直接指標とみなす判定基準一般である。AI benchmark culture や現代科学コミュニティの予測主義は、その観測しやすい実例であって、射程の根拠ではない。

将来、圏論的区別が科学制度や知能システムに完全に内在化され、この混同が制度的に消えるなら、本稿の診断的役割は歴史的足場として後景化する。しかしそれは Predictions Descend Theorem の反証ではなく、対象となる誤配位が消えたという受容史上の変化である。
...
本節は §1.2 C3 ($L$ と $R$ の独立評価) および C5 (3 誤配位の単一起源) の論証骨格である。値問題と構造問題の対称型錯誤を明示することで、§7 の 3 誤配位診断の前提を準備する。

反証可能性が有効に働くのは、主として **個別具体の値問題** においてである。値問題とは、初期条件・境界条件・観測写像を固定したうえで、「この条件下でどの値が出るか」を問う探求である。
...
と置く。このとき、個別の $x,y$ は二つの解の入れ替わりを許すため、一意のラベル付き値としては固定されない。しかし、次の量は条件から一意に決まる:

$$
x^2 + y^2 = (x+y)^2 - 2xy = 3^2 - 2\cdot 1 = 7.
$$

ここで問われているのは、「任意の世界で常に $x^2+y^2=7$ か」ではない。問われているのは、**この制約系の内部でどの量が保存され、どの量が決まるか** である。したがって、この探求に対して「条件非依存の普遍性がないから科学ではない」と批判するなら、それは値問題に構造問題の基準を誤って要求している。

逆向きにも同じ型錯誤が生じる。**普遍抽象の構造問題** とは、「この条件下でどの値が出るか」ではなく、「条件を横断してどの関係・不変量・変換則が保存されるか」を問う探求である。たとえば

$$
x^2+y^2=(x+y)^2-2xy
$$

は、特定の $x,y$ の値についての経験的予測ではなく、任意の $x,y$ に対して成立する恒等的関係である。この関係に対して、「どの観測値が出れば反証されるのか」と問うことは、構造命題を値予測命題として扱うことである。より直観的には、オイラー恒等式 $e^{i\pi}+1=0$ に対し「どの観測値が出れば反証されるか」を問うのと同型である。

したがって、二つの型錯誤は対称である。

| 探求 | 問うもの | 正しい検査 | 誤った要求 |
|:---|:---|:---|:---|
| 個別具体の値問題 | この条件下でどの値が出るか | 値が存在範囲内にあるか、観測と合うか、補助条件と整合するか | 条件非依存の普遍性であることを要求する |
| 普遍抽象の構造問題 | どの関係が条件を横断して保存されるか | 構成的反例、不変量、同型、型整合性、適用境界 | 個別値予測として反証可能であることを要求する |

反証可能性は、値問題における強力な局所検査である。本稿が批判するのは、その局所有効性を主判定基準へ拡張する操作である。ある理論が「この条件下で値 $v$ がこの範囲に入る」と主張するなら、観測値がその範囲から外れることは、理論・射程・補助条件のいずれかを修正させる。これは値問題の正しい運用である。

しかし、構造問題において必要なのは、値予測型の反証可能性ではない。構造問題における検査器の一つとして **構成的反例** がある。これは「すべての群はアーベルである」を $S_3$ という具体的群の構成で破ること、「すべての連続関数は微分可能である」を Weierstrass 関数の構成で破ることのように、普遍命題を反例構造の構成によって反駁する。これは論理形式 ($\exists X, \neg P(X) \Rightarrow \neg \forall X, P(X)$) としては Popper 型反証と同型だが、**検査方法は経験的観測ではなく構造的構成である**。Popper 自身、数学的真理は「a priori であり経験的に反証不能」として科学の範疇から除外しており (Popper 1935)、Popper 型反証可能性は本来的に経験的・値レベルの道具として定義されている。

したがって、構造命題に対する真摯な批判は、Popper 型の経験的反証可能性を要求することではなく、**構成的反例の提示・不変量の破れの指摘・同型の不成立の証明・型違反の摘発・適用境界の越境の指摘** によってなされる。

**構造を批判したいなら、Popper 型反証ではなく、構造的反例を求めよ**。これは弱い批判ではなく、構造命題に対する正しい武器である。$S_3$ が「すべての群はアーベル」を破り、Weierstrass 関数が「すべての連続関数は微分可能」を破ったように、構造命題は構造的反例の構成によって破られる。

本稿の論証核は構造命題である。したがって、それを批判する正しい武器は、$L \dashv R$ 随伴対が成立しない具体的圏の構成、$\eta_{\text{unit}}$ が同型となる具体的反例、5 分野横断同型を破る具体的分野の提示である。経験的予測の外れは、本稿が引用する個別事例や補助接続を傷つけうるが、それだけでは論証核への反証にはならない。

ゆえに、Popper 型反証可能性の過ちは、局所的な値検査としての有効性にあるのではない。その過ちは、値問題に属する検査形式を、科学一般の主判定基準へ拡張した点にある。個別値を問う探求に普遍性を要求するのが型錯誤であるのと同様に、普遍構造を問う探求に値予測型の反証可能性を要求することも型錯誤である。

§7.1 の T9 診断はこの拡張を Bogen-Woodward 関手構造の側から (どの関手段が忘れられたか) として再記述する。本節は値/構造の型錯誤の側から (なぜ拡張が誤りか) を立てる。

---

## §7 corollary (3 誤配位独立節)

§1.2 C5 で予告した「Popper / Mangalam / multiverse predictive underdetermination の 3 誤配位は単一の構造的誤測定から派生する」を、3 つの独立節で展開する。各節は独立に読めるよう設計しつつ、§8 結語で単一図式に閉じる。
...
### §7.3 multiverse への "predictive underdetermination" 批判 (Ellis & Silk 2014)

multiverse (多元宇宙) 仮説への批判の主たる形式は、「multiverse はあらゆる観測と整合する → 何も具体的に予測しない → 科学ではない」というものである。Ellis & Silk 2014 *Nature* op-ed "Scientific method: Defend the integrity of physics" (vol. 516, pp. 321-323) はこの批判の canonical な定式化であり、「予測一意性が失われる → 悪い理論」を判定基準とする。

本稿の関手論的読み替え:

- predictive underdetermination 批判は **理論層の冗長性 → 予測₁ の不一意性** を「悪さ」と判定する
...
> [SOURCE 中: Ellis G., Silk J. (2014) "Scientific method: Defend the integrity of physics" *Nature* 516, pp. 321-323 — subagent 未抽出 (G-ζ); 本節の関手論的読み替えは §1.2 C4 + §6.6 構成的反例 vs Popper 型反証の区別から独立に導出される]

**C4 への帰着 (異型)**: Mangalam が「予測一意性 = 真理₀ 指標」と読むのに対し、predictive underdetermination 批判は「予測一意性が失われる = 理論が悪い」と読む。両者は **同じ誤配位の表裏** である。前者は予測の存在を真理₀ 指標として、後者は予測の不在を真理₀ 否定として、いずれも phenomena 層の独立性を見落とす。

> **本節の射程明示**: 本節は predictive underdetermination 批判の **形式** (予測一意性消失を理論失敗の指標とする操作) を診断するに留まる。multiverse 仮説自体の構造的妥当性については、本稿は独立した判断を保留する。本稿の論証核は批判の category error の形式診断であって、批判対象の擁護ではない。multiverse 仮説 (あるいは類似の構造命題) を批判するためには、§6.6 で示した正しい武器 (構成的反例の構成、内的不整合の証明、構造保存の破れの指摘、適用境界の越境の指摘) が必要であり、それは本稿の論証範囲外である。
...
| predictive underdetermination 批判 | 理論層冗長で予測₁ 消失を「悪い」と判定 | $R$ (真理₀ → 予測₁) の合成を線形視、$L_{\text{th→ph}}$ の冗長性を $R$ 全体の悪さと混同 | 予測一意性の欠如を、Goldilocks 帯の制御面欠落ではなく理論層そのものの失敗として読む。欠落候補を構造的失敗と混同する |
...
| **predictive underdetermination 批判** | 単射性の局所化失敗 | $R_{\text{th→ph}}$ (theory → phenomena) の非単射性を、$R$ 全体 (theory → data) の悪さと混同する |
...
**3 誤配位 (§8.2) は本定理の系として閉じる**: Popper / Mangalam / multiverse predictive underdetermination は、本定理が排除する操作 ($R^{-1}$ の構成、$\eta_{\text{unit}}$ の同型仮定、$R$ の単一線形視) のいずれかに依拠している。3 誤配位は本定理の不可能性に違反する 3 つの誤読であり、本定理から **論理的に閉じる**。
...
§8.3 で構造的不可能性命題として陳述した Predictions Descend Theorem を、Gödel 第二不完全性定理の categorical proof (Joyal arithmetic universe 経路 + Cantor categorical 対角化補題の AU 内擬制) に類比する形式証明として試行する。本節の目的は、形式化の **到達面と境界面** を構造的に開示することにあり、Gödel 級の完全形式証明を達成することではない。届かない部分は §8.4.5 で G-θ' として honest に開示する (本稿の道 C 宣言 §M0.3 「Gödel と並ぶ認識論的定理」との対応で、Round 5 honest 較正後の達成度を **86-91.5%** に固定する。詳細較正は §8.4.4)。
...
| **C5** (3 誤配位の単一系統一) | ◎ Kalon△ | Popper / Mangalam / multiverse predictive underdetermination / ML over-fit / replication crisis (5 派生、全 Future-Proof 強化、§8.2 で関手論的同型に昇格) |
...
2. **3 誤配位の関手論的同型化** (§8.2) — Popper / Mangalam / multiverse predictive underdetermination を **3 つの不正規関手操作** (関手分解の忘却 / 随伴の方向反転 / 単射性の局所化失敗) として形式化、共通根 = 「随伴 $L \dashv R$ を等置する誤読」。C5 が「構造的類似 仮説 55%」(§M3) から関手論的同型 (§8.2) に昇格
...
本稿の射程は、予測を理解または真理₀の直接指標とみなす判定基準一般である。現世代 AI 評価や科学コミュニティの予測主義は主要な実例だが、核命題は LLM 世代に依存しない。
...
- **$R_{\text{paper}}$ (批判解体側、§6-§8)**: 内在化された定義から、3 誤配位 (Popper / Mangalam / multiverse predictive underdetermination) の関手論的同型化と Predictions Descend Theorem の Gödel 類比による不可能性命題を回復する関手
...
- **§6** で立場 (構造決定論 / 予測主義一般 / commitment スペクトル) を限界明示
