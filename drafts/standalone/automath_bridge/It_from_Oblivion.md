...
### 4.3　橋渡し——カテゴリカルシンプレックスが二者を接続する

「空間が根本的に異なる」という反論は正当だ。automath のハイパーキューブ $\{0,1\}^n$ には Fisher 計量も Chebyshev 1-形式もない。忘却論の統計多様体 $M$ にはビット列も Fibonacci 数もない。

しかし、両者を橋渡しする自然な統計多様体が存在する: **カテゴリカルシンプレックス** $\Delta^n$。

Paper I Appendix B は $\Delta^2$ (3 カテゴリの離散確率分布) 上で方向性定理を具体的に計算している:

- Chebyshev 1-形式: $T_i = 1 - (n+1)p_i$
- 均等分布 $(1/3, 1/3, 1/3)$ で $T_i = 0$——盲点のちょうど中心
- 非対称点 $(0.15, 0.15, 0.70)$ で $F_{12} = -2.728$——非自明な忘却曲率が出現
- $dT = 0$ が普遍的に成立 (指数型分布族)——力は方向的不整合 $d\Phi \wedge T \neq 0$ のみから生じる

automath のハイパーキューブ $\{0,1\}^n$ は、各ビット位置をベルヌーイ確率変数と読めば、$\Delta^n$ の頂点の近傍に埋め込まれる。No11 制約は積ベルヌーイ空間上の制約付き部分多様体を切り出す。fold 演算子はこの部分多様体上の粗視化写像である。carry defect は、$F_{ij}$ のこの部分多様体への制限として解釈できる <sup>§</sup>。

<sup>§</sup> この解釈は構成ではなく仮説。厳密な関手 $D: \textbf{Man} \to \textbf{Hyp}$ の構成は open である。ただし、格子ゲージ理論 (Wilson 1974) が連続 Yang-Mills 理論の離散版として 50 年の物理学的実績を持つことを踏まえれば、「離散版が連続版のインスタンスである」という主張形式は方法論的に確立されている。

### 4.4　力の最小単位——2-cell の必然性

忘却論と automath は、もう一つの構造を共有する: **1 次元では力は定義不能**。

忘却曲率 $F_{ij}$ は 2-形式 (反対称テンソル) であり、独立成分数は $\binom{n}{2}$ 個。$n = 1$ なら成分数ゼロ——力の概念そのものが存在しない (Paper V, Th. 3.4.1)。

automath の `deltaSet` と `walshFlux` も同じだ。1 ビットの反転 (1-cell) では離散微分の符号交替が発生せず、`walshFlux` は自明にゼロ。carry defect が非自明な値を持つには、最低 2 ビット (2-cell) が張る面が必要。

直感的に言えば: 方向が 1 つしかなければ「不均一」は定義できない。力は方向の**差**から生まれる。この事実は離散でも連続でも同じだ。

### 4.5　忘却レベルの同定

automath の carry defect は、忘却論の忘却階層 (aletheia.md §5) において $n = 1.5$ (**U_compose**——射の合成律の忘却) に位置する。fold (忘却) と stableAdd (合成) の非可換性は、「合成律の忘却によるドリフト」の離散的な現れだ。射の存在自体は保存されている (`restrictLE` は well-defined)。壊れているのは射の**合成の保存**である。
