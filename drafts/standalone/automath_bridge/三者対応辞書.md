...
| OT-S05-3 | `ZeroForgetCollapse` の地位 — 追加公理か、既存原理からの導出か | axiom boundary | **Partially-open** (2026-04-27 /noe + /ele で精密化) | /noe (2026-04-27) で 3 角度 (FEP / enriched / U⊣N) からの直接導出を試みたがすべて閉じる結果。/ele (2026-04-27) で 4 派生候補経路が露出: OP-S05-3.1〜3.4 (下記)。**現時点判定**: ZFC は α=0 古典圏論文脈に限定した独立公理として採用。各派生候補が completion した時点で派生定理に格下げ |
| OT-S05-3.1 | Lawvere Boolean topos 翻訳経路 | derivation candidate | **Open** | ZFC を「Boolean topos の特徴付け定理」に翻訳する経路。Subobject classifier Ω が Boolean (= 1+1) になる条件が Boolean topos の性質から自動的に従うことを利用 [SOURCE: Mac Lane & Moerdijk *Sheaves in Geometry and Logic* Ch. IV, ただし参照は記憶ベース [TAINT: 記憶], 原典 read 未]。min合成圏 (反例) の subobject classifier 同定と Łukasiewicz 多値論理対応の確定が必要 |
| OT-S05-3.2 | Paper IX α=1 物理的導出経路 | derivation candidate | **Open** | Paper IX OP-IX-7 三段チェーン (μ↓ → α↑ → S_CPS↑) で α=1 (離散圏) 極限を取ると Mor(C_1)(I,X) = ∅ (I≠X) [SOURCE: Paper IX L160-170 §3.5(ii)] により Hom 空間が基底レベルで {0,1} に縮約。ZFC を α=1 限定の物理的派生定理として導出する経路。α=0 (Φ=0 = forgetting なし) との発動条件分離が必要 |
