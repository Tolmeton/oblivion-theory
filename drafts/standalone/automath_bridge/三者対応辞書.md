...
| OT-S05-3.4 | Smithe Bayesian lens 合成経路 | derivation candidate | **Open** | Smithe et al. 2023 "Bayesian updates compose optically" の Bayesian lens 構造で、Φ=0 → agent posterior delta → Giry monad trivialize → Kleisli が Set に collapse → Hom Boolean 化 を導出する経路 [SOURCE: 統一記号表 §4 Smithe ref]。**FEP × enriched の合成経路** で /noe が盲点として認識した経路。lens 構造の厳密構成と Φ=0 入力時の Kleisli collapse 条件の確定が必要 |
