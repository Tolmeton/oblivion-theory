...
**注記 (2026-04-14).** Face Lemma はここでは「圏論版 syndrome 条件」として読むのが有効である。clock transport / minimal closure の対応は、単一 face の detectability を時空方程式へ持ち上げる試みであって、recoverability の十分条件を主張しているわけではない。対応表は `drafts/incubator/FaceLemma_技術設計.md`。
...
| OT-S05-3 | `ZeroForgetCollapse` の地位 — 追加公理か、既存原理からの導出か | axiom boundary | **Partially-open / A+B Dual citizenship** (2026-04-27 第二次更新) | /noe (2026-04-27) で 3 角度 (FEP / enriched / U⊣N) からの直接導出を試みたがすべて閉じる結果。/ele Round 4 で 4 派生候補経路が露出。/ele Round 5 (2026-04-27 第二次) で「Boolean 化」の意味を 3 軸 (値 / 濃度 / indicator) に再分解、ZFC の解釈が 2 候補に分離: **解釈 A (V-enriched [0,1] base)** = 独立公理 (反例 min合成圏が有効) / **解釈 B (Set existence indicator)** = 派生定理候補 (Paper IX §3.5(ii) で導出可能)。Tolmetes 判断で **A+B Dual citizenship** 採用。同一 ZFC が 2 解釈下で異なる地位を持つ (Yugaku Definition Surface Protocol)。OP-S05-3.1〜3.4 は解釈 A 文脈、OP-S05-3.2 は解釈 B 文脈で活きる |
| OT-S05-3.1 | Lawvere Boolean topos 翻訳経路 (解釈 A) | derivation candidate (A) | **Open** | ZFC を「Boolean topos の特徴付け定理」に翻訳する経路。Subobject classifier Ω が Boolean (= 1+1) になる条件が Boolean topos の性質から自動的に従うことを利用 [SOURCE: Mac Lane & Moerdijk *Sheaves in Geometry and Logic* Ch. IV, ただし参照は記憶ベース [TAINT: 記憶], 原典 read 未]。min合成圏 (反例) の subobject classifier 同定と Łukasiewicz 多値論理対応の確定が必要。**解釈 A (V-enriched) 文脈で活きる** |
| OT-S05-3.2 | Paper IX α=1 派生定理経路 (解釈 B) | derivation theorem (B) | **Partially-derived** (2026-04-27 第二次) | **解釈 B (Set existence indicator) 文脈では Paper IX §3.5(ii) からの派生定理候補として既に立つ**: Mor(C_α)(I,X) の存在/非存在を {0,1} indicator で読むと、α=1 で Mor(C_1)(I,X) = ∅ (I≠X) [SOURCE: Paper IX L168] により ZFC が要求する Boolean 化が自動派生。残課題: (i) α=0 (Φ=0) と α=1 (完全忘却) の 2 端点で Boolean 化が同時に成立する条件、(ii) 中間 α における indicator の連続性、(iii) Paper IX OP-IX-7 三段チェーンとの統合。**解釈 A 文脈では別軸 (値 vs 濃度 vs indicator) のため独立公理として残る** |
| OT-S05-3.3 | HoTT / Univalent foundations 経路 | derivation candidate | **Open** | HoTT internal logic で「Hom 空間の (-1)-truncation が Boolean に等しい」を univalence axiom + propositional resizing から派生する経路。Φ=0 を「object identity が strict equality (refl path) に collapse」と解釈。Paper VIII OP-VIII-5 (HoTT 接続) [SOURCE: 統一記号表] の completion 依存 |
| OT-S05-3.4 | Smithe Bayesian lens 合成経路 (解釈 A、(ii) 値軸) | derivation candidate (A) | **Closed (派生不能, 2026-04-27 第三次)** | Smithe (2021) arXiv:2006.01631 "Bayesian updates compose optically" + Smithe (2022) DPhil arXiv:2212.12538 + Tull-Kleiner-Smithe (2024) EPTCS 397 [SOURCE: Paper II 参考文献 [17][18][19]]。/noe+ L3 (2026-04-27 第三次) で 4 段合成経路 ((i) Φ=0 → posterior delta → (ii) Giry trivialize → (iii) Kleisli が Set に collapse → (iv) [0,1] base → {0,1}) を本質分析した結果、**(iii)→(iv) で構造的に閉じない**。理由は **二重防御**: (R5) Kleisli morphism collapse は base monoidal V を変えない (enriched category theory 標準事実) — Set-enriched category で全 morphism が恒等になっても Set 自体は不変、と同型 / (R8) Smithe lens は Poly インデックス、CPS は (ℝ,≤) インデックスでインデックス圏が不一致 [SOURCE: Paper II L1279]。反例 (min合成圏) は probability axioms (countable additivity) を満たさず、Smithe lens 構造を持たない可能性が高い ([0,1] は probability simplex でなく order interval)。Boolean 化 3 軸のうち **(ii) 値軸**を狙う唯一の派生候補だったが構造的に閉じる経路がない。第三の道候補: Tull-Kleiner-Smithe string diagram 拡張 / Poly×(ℝ,≤) 二重インデックス統合 (Paper II §7.5 が示唆) — いずれも未構成 |
