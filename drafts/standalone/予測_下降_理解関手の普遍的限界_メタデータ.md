...
**版**: v1.4 (2026-04-25, Round 5 (§M5.5) 完了 / 道 C 射程 50% honest 較正 / Round 6 課題明示)
**役割**: `Predictions_Descend_理解関手の普遍的限界.md` の F⊣G 台帳 / 核主張レジャー / Gauntlet ログ / 虚→実変換面 を単一ファイルに集約する。
**本体状態**: 本体 `.md` 起票進行中 (v0.1 — §1 7 段開口部 + §2-§8 骨格)。執筆 gate 3/3 全通過 (§M4.1 入口 ✓ / §M5.1+§M5.2 Gauntlet ✓ / §M3 Kalon 全 5 件 ◎ ✓)。
...
| 親ファイル | 本稿での役割 |
|:---|:---|
| `drafts/standalone/エッセイ_理解と予測の随伴.md` (v1.4.0) | §2-§5 の随伴定理部分の種 |
| `drafts/standalone/反証可能性は死んだ_エッセイ.md` (v5.6.0) | §6 corollary (Popper) + §7 corollary (超ひも landscape) の種 |
| `00_核心｜Kernel/A_公理｜Axioms/E_形式化｜Formalization/fep_epistemic_status.md` | 真理₀/真理₁ 区別の正本 |
...
| `00_核心｜Kernel/A_公理｜Axioms/E_形式化｜Formalization/oblivion_connection_map.md` | §2.4 普遍フィルトレーションの分類定理 (L179-L263) — Mac Lane/Riehl/Kelly/Johnstone 写像 [推定 90%]、r3/r8 (Z-03 圏論 overclaim) 外部強化材料 |
...
- **試行**:
  ...
  - fep_epistemic_status.md §予測の二層分解 v2.5.0 (L288-L301) [SOURCE 再 Read 確認済]:
...
- **試行 (内部 SOURCE で外部接続を借りる)**:
  - **oblivion_connection_map.md §2.4 (B-3 普遍フィルトレーションの分類定理) [SOURCE 再 Read 確認済 2026-04-25, L179-263]**: U_arrow ≤ ⋯ ≤ U_self の 8 段に対し:
  ...
  - 本稿 r3 への作用: **構造保存定理 (Paper VII) は HGK 内部閉鎖ではなく、Mac Lane/Riehl/Kelly/Johnstone 4 教科書への 90% 写像済 [SOURCE: oblivion_connection_map §2.4 L190]**。外部 reviewer は「HGK 内部に閉じている」と判定できない
...
- **実化操作**:
  - oblivion_connection_map §2.4 [SOURCE] を本稿 §3 Paper VII 構造保存定理導入時の **外部接続 anchor** として引用 (定理引用追加)
...
| 対応 | 状態 | SOURCE |
|:---|:---|:---|
| r3 (C2) | **段階 A 完了 ✓** | oblivion_connection_map §2.4 [SOURCE: 再 Read 確認済] |
...
| 実へ引く SOURCE | **aletheia §7.4 忠実性の証明 (L2601-L2706) [SOURCE: faithful but not full の Mac Lane 言語化済]** / **aletheia §7.7 Phase 6 Yoneda が射と自己を結ぶ (L2790-L2828) [SOURCE]** / **oblivion_connection_map §2.4 (L179-L263) [SOURCE: Mac Lane/Riehl/Kelly/Johnstone 写像 推定 90%]** / Paper VII §6.1-6.2, Faithful functor factorization, Kan 拡張 (Mac Lane Ch. X) |
...
| 実へ引く SOURCE | **aletheia §1 随伴定理 U0' (L99-L107) [SOURCE: F[N(q_poor)] ≤ F[q_poor] の VFE 減少定理]** / **fep_epistemic_status.md §予測の二層分解 v2.5.0 (L288-L301) [SOURCE: 補完₁ 定義]** / fep_epistemic_status.md §2, Tishby-Pereira-Bialek 1999, DPI (Cover-Thomas Ch. 2), Gaussian state-space model |
...
| 現在まだ虚な点 | 真理₀/真理₁ 区別は fep_epistemic_status v2.6 で導入。科学哲学既存区別 (法則/現象 / 理論/モデル / 抽象/具体) との対応が未接続 |
| 実へ引く SOURCE | **fep_epistemic_status.md §真理と予測の型分け v2.4.0 (L191-L220) [SOURCE: 真理₀/₁/予測₀/₁ 完全定義]** / **fep_epistemic_status.md §ポパー適用不能の最小主張 v2.7.0 (L234-) [SOURCE]** / Paper VII / Cartwright (1983) / van Fraassen (1980) / Bogen-Woodward (1988) (外部接続は G1 ギャップとして未着) |
...
| 実へ引く SOURCE | **fep_epistemic_status.md §ポパー適用不能の最小主張 v2.7.0 [SOURCE]** / 反証可能性.md §8 §8.6 §9-10 / Hossenfelder (2018) / Popper (1934, 1959) |
...
- `03_忘却論｜Oblivion/drafts/infra/リファレンス/忘却論オンボーディング.md` — 忘却論タスク開始時に毎回 Read する
...
1d. **§M8.3 Layer 2 ★★★+★★ 4 ファイル精読** ✓ (2026-04-24, aletheia §1/§7.4/§7.7/§2.1/§2.3, fep §真理予測型分け/§予測の二層分解/§ポパー適用不能, oblivion_connection_map §2.4)
...
3. **統一記号表更新** ✓ (2026-04-25, 案 A+B 併用で実行完了): §1.13 Predictions Descend 固有記号 (L/R/η_unit/Ker(η_unit)/F_div/U_understand 6 項) を §1.11 と §1.12 の間に新設、§1.12 衝突表に η 行新規追加、F 行に F_div を追加、U 行に U_understand を追加。所在は `03_忘却論｜Oblivion/drafts/infra/リファレンス/統一記号表.md`
...
11. **本体 `.md` 起票** (進行中, /exe+ Round 4 必要): §1-§7 v0.2 起票完了 (408 行)。/exe+ で 🔴 3 件 + 🟡 8 件検出 → /dio (L2 fix) で **公理→構成的定義降格 / §5.4 単調減少定理→Lagrangian 形式化降格 / §6.4 残ギャップに G-η (5 分野形式同型射) / G-θ (Gödel 級不可能定理) / G-ι (IIT SOURCE) / G-κ (Bogen-Woodward 過度依存) を新設 / §4.5 で Yoneda 論理飛躍を明示開示** を実行。残作業: **Round 4 を §M5.4 として新設し、G-η/G-θ/G-ι/G-κ を gauntlet 形式で処理 → Round 5 で道 C 射程の honest 較正 → §M3 Kalon 判定再実行 → §8 結語起票** の順序
...
| **G4** ω 折畳の形式証明 | **本論文外問題として §6 で限界明示 (Round 2 段階 A で確定)** | oblivion_connection_map §2.4 で構造的開問題として開示済。本稿は ω 折畳 gap を含む CE=B[推定 90%] の上で構造保存定理を引用 |
...
| **G-ε (新)** Tishby 1999 Gaussian 閉形式の TAINT 残 | 未解消 (SSL cert error) | 本体起票時に `wget --no-check-certificate https://www.princeton.edu/~wbialek/our_papers/tishby+al_99.pdf` 等で再取得、または arxiv physics/0004057 v2/v3 から self-consistent eq の specific form 確認 |
...
| **G-η** (Round 4 由来) | 5 分野同型の形式同型射構成 | **部分着手 ✓** (Round 4 で 2 ペア骨格: Information Geometry × Statistical Mechanics の Legendre 双対 + Gauge × Information Geometry の Yang-Mills/Fisher 曲率対応)。5 分野全ペア (10 対) natural transformation 完全構成は Round 5 課題 |
...
| **G-ι** (Round 4 由来) | IIT (Tononi 2008 / IIT 4.0 2023) 同型 SOURCE 確保 | **部分着手 ✓** (2026-04-25 alphaXiv MCP triangulation 取得済): IIT canonical claim = 「consciousness is identical to a system's integrated causal structure, an irreducible cause-effect repertoire quantified by Φ (phi), intrinsic property of the system」[SOURCE 中: arxiv 2510.04084 v1 p.1, alphaXiv triangulation]。IIT commitment = **ontological** (consciousness identical to causal structure, 必要十分条件) と本稿 = definitional (operational anchor) の差が SOURCE で確定。完全 PDF Read による「強候補」昇格は Round 5 課題。**追加発見**: arxiv 2509.00555 "Integrated information and predictive processing theories of consciousness" (Albantakis 2025-08-30) が IIT × predictive processing の比較レビューとして直接関連、Round 5 で精読推奨 |
...
| **G-λ** (Round 4 由来 / Round 5 で「停滞 △」確定) | NRFT 完全形式証明 | **停滞 △** (Round 4 骨格 §5.6 維持、Round 5 で本セッション内未実行確定)。定理ステートメント + 証明 + 系 + Gödel 第二不完全性との形式的同型は Round 6 課題 |
...
*v0.3 — 2026-04-24 Layer 0/1/2 ★★★+★★ 完了に伴う SOURCE 反映。§M0.2 に aletheia.md と oblivion_connection_map.md を親材料として追加。§M1.4 に L⊣R↔U⊣N 同型対応行と F⊣G 別レベル明示行を追加。§M6 C1-C5 の SOURCE 行を Layer 2 確定パス (aletheia §1/§7.4/§7.7/§2.1/§2.3, fep §真理予測型分け/§予測の二層分解/§ポパー適用不能, oblivion_connection_map §2.4) で更新。C2 最新状態を「変換中」→「実 (Mac Lane 言語化済)」に格上げ。§M9 進捗 ✓ 反映と §M9.1 残ギャップ表を新設。*
