...
- **棄却 5**: 「Cauchy 位置スケール族を反例として dT ≠ 0 の非指数型を例示する」。数値 pilot で T ≈ 0, dT ≈ 0 を返し、閉形式計算でも $C_{abc}\equiv0$、従って $T\equiv0$, $dT=0$ と確定した。反例としては機能しなかったが、「非指数型 ⟹ dT ≠ 0」という一般命題が成立しないことの exact witness として OP-I-7 に記録。
...
| theorem seed | OP-I-7: $dT=0$ の保持 / 破れの class 分類 | §5.4 / Appendix F.6 に working classification 挿入済み。Student t N1 は finite-difference stability bound まで昇格済み。Cauchy N0 は閉形式で $C_{abc}\equiv0$ まで昇格済み | E0 exact-closed、N1 twist-positive、N0 twist-dull を同じ class-level invariant で分ける。次は Student t の形式的 interval arithmetic / 解析的境界と、Cauchy の $C\equiv0$ を生む symmetry / moment-cancellation criterion の抽出。 |
