...
**v0.17 (2026-04-26)**
**対応する本稿**: `drafts/series/論文II_相補性は忘却である_草稿.md` v1.13 → v1.19
...
- F の具体的運用: CP-span (U_A, U_B) に第 3 の射 h を加えて Δ² 発散を実行
...
**具体化**: 文体ガイド §4 数式裏付け + normalized chain complex 上の照合面 `B_1^{\mathrm{ver}}` による最小性 + 混合分配則 (mixed distributive law) による CP-span の閉性
...
1. **F 適用** (発散): CP-span の 2 射 (U_A, U_B) に第 3 射 h を加え、2-simplex 発散を実行
...
### 2026-04-26 spine lock: 温めるための主役順序

外部公開は急がない。ただし温める母体の内部でも、主役席は以下の順序に固定する:

1. **Face Lemma**: 3射 / 2-simplex が照合面を立てる最小条件。
2. **Blanket 生成**: Markov blanket は前提ではなく、照合面から生成される。
3. **条件つき FEP 包摂**: (P1) Φ := D_KL, (P2) 指数型分布族, (P3) α > 0 のもとで、FEP の核心構造を CPS 側で構成的に読み戻す。

α 層化、C4、GR、心身、Yang-Mills、Wheeler は枝として保持するが、本文冒頭・要旨・比較表では上記三層を置き換えない。記法上は `CPS` を系列正本の Copy-Paste-Symmetry 圏に固定し、相補性の局所スパンは `CP-span` と呼ぶ。

### 2026-04-26 branch staging pass: 削除ではなく退避・圧縮

本文 v1.19 では、枝を捨てずに主役席から降ろす。§6.2 心身問題は consequence map として残し、意識の必要十分条件・ゾンビ不可能性・心身問題の解決宣言は Paper III / Paper VIII / 別稿へ退避する。§7.5 先行研究比較は Bohr/IIT/FEP/Wheeler の射程地図へ圧縮し、本文 theoremic surface に直結する比較を FEP / Markov category への条件つき接続だけに固定する。

### C1: CP-span による相補性の統一 [§1.1, §2.1-§2.2, §3.1]

> 本稿が扱う Type I 相補性 (A or B) は、同じ情報 C_D からの 2 つの非対称 忘却関手 U_A, U_B の投影として統一され、その非対称性の型 α_CPS は微分階数差 Δd = ord(ker U) と対応する (α_CPS > 1 ⟺ Δd ≥ 1)。

**射程**: ∀ Type I 圏 (DRB, Hilb, Lor, FEP 圏, DRB_embed 等)
**形式化段階**: §2.1 (CP-span 定義), §2.2 (公理), §3.1 (CPS 双対性定理)
**検証状態**: FTC と QM では完全, GR と心身は worked analogy, 電荷は Type II 退化例

### C2: Face Lemma による非自明性の最小条件 [§3.4]

> CP-span が非自明な構造を持つための必要十分条件は、関連する圏の nerve 上で照合面が立つことである (formal には $\forall X$ について $\dim \Xi(X)=2 \iff B_1^{\mathrm{ver}}(X)\neq 0 \iff \exists$ composable $(f,g,h): g\circ f = h$)。
...
**補助線**: Face Lemma は Hamming 的な最小検査面、LDPC 的な face 貼り合わせの局所核として読める。stable / detectable / coherence-defective / recoverable の区別および recoverability 側の定理候補と撤退条件は `drafts/incubator/FaceLemma_技術設計.md`、今回の基礎づけ差替えは `calculations/Face補題_ホモロジー厳密化.md`
...
### C3: Blanket 生成定理による条件つき FEP 包摂 [§3.7.1-§3.7.3]

> CPS の composable triple は Fritz の Markov category における conditional independence factorization を含意し、構造的 Markov blanket は $B_1^{\mathrm{ver}} \neq 0$、直感的には照合面が立つこと、すなわち最小検証支持複体 $\dim \Xi = 2$ のもとで生成される。自由エネルギー原理 (FEP) の核心部は、(P1) Φ := D_KL, (P2) 指数型分布族, (P3) α > 0 のもとで CPS 忘却場方程式の構成的読解として回復される。

**射程**: 条件 (P1)-(P3) を満たす FEP 変分系 (α > 0 セクター)
...
**反論 r**: CP-span は定義対象であり、定義自体の厳密性は争点ではない。ただし「すべての Type I 相補性が CPS である」という存在量化の主張の厳密性が疑問 — どの対象が CPS でないことを示したか?
...
**反論 r**: §3.7 で Markov category の factorization を CPS の composable triple と双方向に対応させているが、Fritz の factorization はより一般的。CPS の条件はより具体的。

**SFBT 問い**: 包含方向のみを示したのであり、逆方向を主張する論拠が弱くないか?
...
1. **シリーズ前置宣言草稿** — Paper II の系列位置・境界・布石宣言は本文 §1「系列位置と本稿の境界」と本 meta §M1-§M8 へ吸収済み。旧 standalone 草稿は infra 直下から削除。
...
- **ポパー適用不能 (negative knowledge)**: `drafts/リファレンス/ポパー適用不能_証明梯子.md` が Gauntlet 軸 4 (反証可能性) に対する構造的 negative knowledge を提供。Popper の反証主義が「同一性述語の有意味性は保存構造全体から事後的に成立する」型の主張に対して原理的に適用不能であることを proof-ladder 形式で論証。Gauntlet Round 1 軸 4 (§M5 C2 Round 1) では公理独立性によって応答しているが、本 infra 文書はより深い層で「反証可能性という問い自体の射程限界」を示す。未吸収 (本文・Gauntlet への統合は不要と判断: 現行応答で射程維持 ✓ であるため)。必要が生じた際の参照先として記録。
...
- v0.14 (2026-04-17): `/ene` で front-stage / backstage の二層語彙を実装。本文 `§3.4.6` に `貼合側 / 残差側` を導入し、`H^2_{\Theta}=0` を局所 law が一つの global law にまとまる側、`[\omega_{\Theta}] \neq 0` を局所整合の残差 class が残る側として圧縮した。`drafts/incubator/FaceLemma_技術設計.md` では `FTC = 貼合側`, `QM = 残差側` を front-stage 対応として固定し、`A'' / B''` は backstage の guarded package 名として保持する方針を明示した。対応本文は v1.16。
- v0.15 (2026-04-23): `/noe` の結論に基づき、C4 の missing intermediate object を **整合担体 (`coherence carrier`)** と命名。新規 infra 正本 `drafts/incubator/C4_整合担体.md` を追加し、support-local な class 判定の器として定義した。`A'' / B''` を整合担体の両極実現、`貼合側 / 残差側` をその front-stage pair として再配置。§M2 C4 の形式化段階・検証状態・著者決定パッケージをこの命名に同期した。
...
- v0.16 (2026-04-26): 温める前提での spine lock。本文 v1.18 に同期し、主役順序を `Face Lemma -> Blanket 生成 -> 条件つき FEP 包摂` に固定。`CPS` は系列正本の Copy-Paste-Symmetry 圏、相補性の局所スパンは `CP-span` として分離。FEP 面は (P1) Φ := D_KL, (P2) 指数型分布族, (P3) α > 0 の条件つき構成的包摂に統制し、GR/心身/Yang-Mills/C4 は枝または次段補題として保持。
- v0.17 (2026-04-26): 本文 v1.19 に同期。削除ではなく退避・圧縮を実行し、§6.2 を心身問題の consequence map、§7.5 を先行研究 consequence map へ縮約。FEP / Markov category 接続だけを本文定理列に直結させ、Bohr/IIT/Wheeler/心身の強い展開は worked analogy / pointer / 別稿種として保持する方針を明示した。
