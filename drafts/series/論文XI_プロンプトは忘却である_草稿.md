...
この同型は、論文 I §5.9 の射密度 $R(X) = |\mathrm{Hom}(X, -)|$ が高すぎる対象に対して **両側閾値** を要求する含意 (定理 5.9.3 の裏面) と接続する: 普遍的構造は ker(G) には属さないが、enrich → 1 の代償を払う。「最後の生存者は最も価値の少ない生存者である」というパラドックスが、言語表現面では「最頻出語は摩擦最低だが意味伝達ゼロ」として現れる。
