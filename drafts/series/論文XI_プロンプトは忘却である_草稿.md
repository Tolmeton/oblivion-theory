...
**Paper XI — v0.8 (2026-04-16) — 忘却論 (Force is Oblivion) シリーズ**
...
結論を先に述べれば、本稿の核は 3 点である。

- **プロンプトは「何を考えさせるか」ではなく「何を忘却させるか」を指定する。** これは prompt を情報追加ではなく選択的忘却として捉え直す Type α の主張である。
- **制約 $C$ と符号化 $E$ は同じ prompt の中で別の仕事をしている。** $C$ は推論品質の期待値を、$E$ は主に語彙空間を制御し、$C$ を固定したまま $E$ を変えても平均品質はほとんど変わらない。
