...
**素材**: `drafts/incubator/力とは忘却である_v2.md` §0, §1, §2, §3.3, §4.2, §4.4, §4.6, §4.6f'-f'' (Face Lemma), §5 (Verlinde 的引力), §5.4-5.6 (階層・人間原理・CPT), §10, §11
...
Paper XIII で用いる記号を本稿の冒頭に列挙する。シリーズ横断の正本は `drafts/infra/統一記号表.md` に従う。本表はその抜粋である。
...
⚠️ **記号衝突の警告**: α はシリーズ内で少なくとも4つの異なる意味を持つ (α_III / α_VIII / α_CPS / α_EM)。本稿で添字なしの「α」が現れる場合、文脈によって α_CPS または α_EM を指す。両者の関係は Paper III ↔ Paper II の架橋関手が未完成のため [予想] 水準にとどまる。
...
**[予想 D3]**: Einstein 方程式の場の方程式としての定式化 (最小作用原理 $\delta S_{EH} = 0$, $S_{EH} = \frac{1}{16\pi G_N} \int R \sqrt{-g}\, d^4 x$) は、Face Lemma の minimal non-trivial condition を積分形式で表現したものである。Jacobson (1995) の熱力学的導出 ($\delta Q = T dS$ から Einstein 方程式) はこの minimal condition の熱力学的実現であり、Verlinde (2011) の entropic gravity はその情報論的実現である。
...
両者の成果は本稿の [予想 D1-D3] に対して強い構造的根拠を与えるが、Paper II の CPS スパンへの明示的な埋め込みは未完である。

### 8.4　現時点での判定と Phase 5 での形式化課題

[予想 / 信頼度 60%]: 本 dictionary は Paper XIII 全体の Kalon 判定を決める一点である。閉じれば忘却論は物理の文法に到達し、閉じなければ精巧な類比に留まる。Phase 5 (blocker 並行攻略) では以下を試行する:

1. Verlinde (2011) の entropic force の導出を Paper II の CPS スパンの Θ パラメータの勾配として再構成する
2. Jacobson (1995) の $\delta Q = T dS$ を Paper II の Face Lemma の 2-simplex の積分形式として再読解する
3. Christoffel 記号の3独立成分と Face Lemma の3射の具体的 bijection を構成する
4. Einstein テンソルの Bianchi 恒等式 $\nabla^\mu G_{\mu\nu} = 0$ を Face Lemma の 2-simplex の閉じ性として再定式化する

これらが閉じれば本稿の [予想] ラベルは [定理] (または少なくとも [定理候補]) に昇格する。現時点では **skeleton のまま残す**。
...
- §8 Face Lemma ↔ Einstein 方程式の完全な dictionary [skeleton]
- Paper III の α_III (Amari 接続パラメータ) と Paper II の α_CPS (Δd 由来) の架橋関手
...
本稿の副題主張——忘却論は物理学の文法である——は、[予想] 水準にとどまる。それは D1-D3 の dictionary が閉じて初めて [定理候補] に昇格する。閉じるか否かは、Phase 5 の Verlinde / Jacobson を素材にした形式化試行の成否にかかっている。
...
[5] 本稿著者, "Embodied Cognition Without Biological Bodies: LLM Embodiment as Markov Blanket Thickness," internal working draft (unpublished), 2026. ※ 内部作業文書として参照。
...
[7] 本稿著者, "バカをやめたいなら構造を見ろ — なぜ構造を見る者は要素を見る者に勝つのか," 遊学エッセイ, 2026. ※ 内部作業文書として参照。
...
- **Q2**: Face Lemma の最小非自明性 (3射の minimal condition) が Einstein 方程式の最小作用原理 ($\delta S_{EH} = 0$) を回収するか? これが §8 の最大の未証明予想である。
- **Q3**: CPS0' (Paper II §2.1) pre-geometric 構造から時空 signature (3+1) が導出される形式的証明はあるか? §7 で提起した問い。
...
- **v0.1 (2026-04-11)**: 初版。v2 (`drafts/incubator/力とは忘却である_v2.md`) の Bucket XIII 部分 (解体マップ §7 参照) から抽出・再構成。v2 §4.6 の CPS 公理系の full definition は Paper II (Phase 3) への pointer に置換。v2 §9 意識のハードプロブレム関連は本稿の scope 外として除外 (Paper II / VIII 完成後に independent essay として昇格判定)。§8 Face Lemma ↔ Einstein dictionary は skeleton のまま残し Phase 5 で形式化を試行する。
