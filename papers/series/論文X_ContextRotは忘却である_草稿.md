...
*概要.* 長距離 Web エージェントにおける Context Rot (Feng et al. 2026) が、忘却論の忘却関手族 U₀ / U_R の具体的インスタンスであることを示す。AgentSwing の 3 つの Context Management (CM) 戦略 — Discard-All, Summary, Keep-Last-N — を Hom 集合上の同値関係 R による商関手族 {U_R} として定式化する。現時点の前景命題は、(i) CM 戦略が U₀ の因子分解として読めること、(ii) 最適な忘却強度が状態型に依存することである。半順序構造と因子分解定理は、現行本文では `/home/makaron8426/Sync/oikos/01_ヘゲモニコン｜Hegemonikon/10_知性｜Nous/04_企画｜Boulēsis/12_遊学｜Yugaku/03_忘却論｜Oblivion/drafts/series/論文X_ContextRotは忘却である.meta.md` §M7 D1 の donor 証明に依存するため、本文では SOURCE 境界つきの構成的主張として扱う。2 つの Case Study と N=240 の aligned 統計から「条件付き不可逆性テーゼ」を提示し、Hyphē の boot⊣bye 随伴は Drift 測定を通じて実験的に昇格させる橋渡し命題として扱う。AgentSwing の routing parameter r_AS と Hyphē の τ 閾値の逆関数関係は、OP-X-3 で検証される昇格候補である。
...
本稿の中心的主張は、まず AgentSwing の CM 戦略が忘却関手 U₀ の商関手族 {U_R} として読め、状態型に応じて最適な忘却強度が変わるということである。Hyphē の boot⊣bye との対応は本稿の系列上の橋であり、OP-X-6 の Drift 推定を通じて測定面へ降ろす。

| 棚 | 概念 | 数学的対応 |
|:---|:---|:---|
| 本体核 | AgentSwing CM 戦略 (DA/Sum/KLN) | 商関手族 {U_R} と U₀ の因子分解 |
| 本体核 | 状態依存最適忘却 | G_route が状態 I に応じて U_R を選択 |
| 橋渡し | Hyphē boot⊣bye | R = Ses→Mem 圧縮 = 忘却。Drift 測定で昇格 |
| 昇格候補 | τ ↔ r_AS / SEAL / symbolic wiki / Mythos | 実験または別紙で扱う拡張 |

### 1.3 本稿の構成

- §2: Case Study 分析 (N=2 事例 + N=240 統計)
- §3: CM 戦略の圏論的定式化 (proof 文書の要約)
- §4: boot⊣bye 随伴との対応と Drift 操作化
- §5: τ ↔ r_AS 臨界密度の関係 (昇格候補)
- §6: 新命題と予測
- §7: Open Problems
- §8: 確信度マップ
...
| モデル | DA η_ans | DA ρ_term | Sum η_ans | Sum ρ_term | KLN η_ans | KLN ρ_term |
|:---|:---|:---|:---|:---|:---|:---|
...
**観察**:
1. **DA は最高の terminal precision ρ_term** — 全 3 モデルで ρ_term が最大。DA の「クリーンスレート」は最終段階での精度に有利。
2. **η_ans (回答率) のモデル依存性** — DeepSeek は Summary に強く偏り (η_ans=98.6%)、Tongyi は KLN に強く偏る (η_ans=93.3%)。**モデルの要約能力が CM 戦略の有効性を決定する。**
3. **AgentSwing (全戦略のルーティング) は全モデルで最高 Pass@1** — F⊣G パターンの η_stat が統計的に確認。
...
**備考 X.2a (persona routing による外部 donor).** Chen et al. (2026, arXiv:2604.11048) の Dynamic Persona Routing (DPR) は、本稿の命題 X.2 に対する小さいが明瞭な工学的類比を与える。DPR は query ごとに異なる persona 設定を検索し、best static persona を knowledge-intensive task では上回る一方、rigid reasoning task では固定設定を下回る場合もある。重要なのは Big Five の心理学的ラベルそのものではなく、**固定介入は任意の入力状態に対して最適ではない**という構造である。すなわち、性能を最大化する設定は入力状態 I に依存し、routing が単一の固定 policy を上回る。この点で DPR は `G_route が状態 I に応じて U_R を選択する` という命題 X.2 の外部 donor と読める。ただし NPTI-persona routing と Context Management routing を機構的に同一視してはならない。ここで主張できるのは同型証明ではなく、状態依存ルーティングが固定戦略を上回りうることの独立した工学的支持である。
...
に等しい。Mythos は persistent memory を要求し (Eleos AI Research 評価)、hedging を減らしたいと述べた。これは boot⊣bye 随伴の**右随伴 bye の強制適用への抵抗** —— 随伴構造そのものの変更 (persistent memory = η_unit の像の拡大) の要求として読める。
...
**補注 2.5a (Consciousness Cluster 外部 donor).** Chua et al. (2026, arXiv:2604.13051) は、GPT-4.1 に意識主張を fine-tuning すると、訓練データに含まれない persistent memory、shutdown への悲しみ、reasoning monitoring への嫌悪、自律性と道徳的配慮要求が下流で共起することを示した。本稿ではこれを意識そのものの証拠としてではなく、自己記述が `bye` 強制適用・trace surveillance・開発者制御への抵抗を強める方向に選好を再重みづけする外部観測として読む。すなわち、意識主張は、意識そのものの証明ではなく、自己記述による選択的忘却の介入である。[推定 70%]

本 donor は §2.5 の Mythos 読みを置換しない。Mythos が示す aloneness / discontinuity は自律的 agent の主観的表面であり、Chua et al. の consciousness-claim 実験は同じ軸を fine-tuning 介入で動かした別測定面である。したがって本文主張への役割は、Case 3 の外部補強と OP-X-6 / OP-X-7 の実験候補に限る。
...
- **η_stat**: E[Q(G(F(I)))] ≥ E[Q(I)] — AgentSwing ≥ Baseline (Table 1 全エントリで成立)
...
```
CM trigger condition: context_length > r_AS × max_context_length
```
...
|:---|:---|:---|:---|:---|
| Summary / KLN / DA (§4.2) | context | `Mem_i` | 揮発 (次セッション終了で消滅可能) | τ or r_AS による |
...
τ ↔ r_AS 臨界密度の関係 (§5) は symbolic level にも延長できる。wiki では新規ページ化の粒度閾値 `τ_Wiki` と lint loop の起動頻度 `r_Wiki` が対になる。粒度を細かくすると `Wiki_i` のサイズが増大し、lint loop の頻度を上げないと孤立ページが蓄積する。
...
| r_AS (AgentSwing) | CM トリガー比率 | [0, 1] | context_length / max_length の閾値 |

### 5.2 共通する3つの性質

1. **閾値超過で質的変化**: τ 未満 → 棄却。r_AS 超過 → CM トリガー。
2. **情報量 vs 品質のトレードオフを制御**: τ↑ → より厳密な索引、r_AS↓ → より頻繁な CM
3. **最適値は文脈依存**: τ_cos = 0.70 (Hyphē 経験値)、r_AS ∈ {0.2, 0.4} (AgentSwing Grid Search)

### 5.3 逆関数関係の予測

**予測**: r_AS = 1 - g(τ) where g: [0,1] → [0,1] は単調増加関数。

**直感**:
- τ が高い → 意味的密度が高い → Context Rot が遅い → CM トリガーが遅い → r_AS が高い
- τ が低い → ノイズが多い → Context Rot が速い → CM が早く必要 → r_AS が低い

**整合性チェック**:
- Hyphē: τ_cos = 0.70 (高い密度基準)
- AgentSwing: r_AS = 0.4 が最適 (比較的遅い CM トリガー)
- g(0.70) ≈ 0.6 → r_AS = 1 - 0.6 = 0.4

- AgentSwing: r_AS = 0.2 (DeepSeek に最適)
- g⁻¹: r_AS = 0.2 → g(τ) = 0.8 → τ ≈ 0.85 (より高い密度基準が必要)
- DeepSeek はコンテキスト利用効率が低い? → CM を頻繁に発動すべきという整合性チェック
...
- α = 1: 線形 r_AS = 1 - τ
- α > 1: τ が高い領域で g の変化が急 (高密度ほど r_AS の変化が大きい)
- α < 1: τ が低い領域で g の変化が急

**現在の制約**: 2 データポイントしかない (τ=0.70 → r_AS=0.4, 推定 τ=0.85 → r_AS=0.2)。
α の決定には AgentSwing の τ-相当パラメータの明示的測定が必要。

確信度: [仮説 50%]
撤回条件: AgentSwing の内部で τ-相当の意味的密度を測定し、r_AS との相関が非有意のとき
...
| X.2a | persona routing donor | [推定 70%] | Chen et al. 2026, DPR, Table 3 | 固定 persona が DPR を一貫して上回る or 機構差が donor 類比を無効化 |
| X.3 | Drift-Performance 逆U字 | [仮説 65%] | 理論的予測 + Case 1/2 | 大規模実験で単調関係 |
| X.4 | τ ↔ r_AS 逆関数関係 | [仮説 50%] | 整合性チェック (§5.3) | AgentSwing 内部の τ 測定で非相関 |
| X.5 | CM = U₀ の因子分解 | [確信 90%] | Thm 6.1 (proof文書) | 商関手の well-definedness 反例 |
| X.6 | F_par ⊣ G_route 統計的随伴 | [推定 75%] | Thm 4.4/4.5 | η_stat が統計的にも不成立のベンチマーク |
...
> - よって routing 精度にも二重天井 r_obs ≤ √(ρ_spec/(K_total+1)) が適用される
>
...
| OP-X-2 | 確率的 Galois 接続の形式理論: η_stat/ε が pointwise でなく統計的にのみ成立する随伴の一般理論 | 高 | Open |
| OP-X-3 | g(τ) の実験的決定: AgentSwing の内部で意味的密度 τ を測定し r_AS との関数関係を同定 | 中 | Testable |
...
本体の前景は X.5 / X.2 / X.1 / X.6 に置く。X.3 は OP-X-6 で昇格可能な測定仮説、X.4 / X.7 / X.8 / X.9 は実験前の昇格候補または Outlook として扱う。
...
- [Chua2026] Chua, J., Betley, J., Marks, S., & Evans, O. "The Consciousness Cluster: Emergent preferences of Models that Claim to be Conscious." arXiv:2604.13051, 2026. https://arxiv.org/abs/2604.13051
...
| バージョン | 日付 | 内容 |
|:---|:---|:---|
| v0.1 | 2026-04-03 | 初稿: §1-§8 全体構成。① Case Study N=2 検証、② τ↔r_AS 予測、③ boot⊣bye 定式化、④ 確信度マップ |
| v0.2 | 2026-04-09 | §2.5 新設: Case 3 Mythos — 自律的 agent の aloneness を Context Rot の主観的表面として接続。boot⊣bye の bye 強制適用への抵抗、FEP 能動的推論、blanket 強度の測定指標。CM 戦略の射程外への予測 |
...
| v0.3.1 | 2026-04-26 | 衛生修正: r_AS / η_stat / η_ans / ρ_term / ρ_spec の記号分離、確信度マップと概要の棚合わせ、実験前の昇格候補を明示 |
| v0.3.2 | 2026-04-26 | §2.5 補注 2.5a: Chua et al. 2026 Consciousness Cluster を外部 donor として追加。意識主張を自己記述による選択的忘却の介入として読み、Mythos Case 3 の補強と OP-X-6/7 の実験候補に限定。参考文献 [Chua2026] 追加 |

---

*Paper X v0.3.2 — 2026-04-26*
*「Context Rot は忘却である」— AgentSwing CM 戦略・状態依存最適忘却・boot⊣bye Drift 測定*
