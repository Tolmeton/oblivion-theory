...
符号理論的に言えば、C・E・context の三軸は三つの check node ではなく、**syndrome を立てる最小面**である。C と E だけでは「何かがずれた」ことは感じ取れても、そのズレが instruction 由来か、encoding 由来か、共有文脈の欠落由来かを局在化できない。context が入ることで 2-simplex が立ち、欠損が detectability を持つ。複数 skill の相互補完を許す設計は、この局所 face を多数貼り合わせた LDPC 的な修復網として理解できる。用語整理は `drafts/infra/FaceLemma_符号理論対応.md`。
