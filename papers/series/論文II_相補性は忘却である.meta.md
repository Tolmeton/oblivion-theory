...
**補助線**: Face Lemma は Hamming 的な最小検査面、LDPC 的な face 貼り合わせの局所核として読める。stable / detectable / coherence-defective / recoverable の区別は `drafts/infra/FaceLemma_符号理論対応.md`、今回の基礎づけ差替えは `calculations/Face補題_ホモロジー厳密化.md`、recoverability 側の定理候補と撤退条件は `drafts/infra/FaceLemma_修復可能性設計.md`
...
**主張水準**: **conjectural**。Paper II 本文では一般 theorem としては立てない。QM Heisenberg instance は最小非自明インスタンスの第一候補だが、本稿の canonical surface ではまだ「既知結果の援用による partial theorem」までは主張しない。

**射程 (canonical)**: Θ が局所 1-cocycle law として組織される CPS ドメイン
**射程 (未検証)**: 最小非自明インスタンスとしての QM / FTC / その他具体例

**形式化段階**: §3.4.6 本文草稿まで実装済み。主定義は $Z^2_{\Theta}=\ker d^2$, $B^2_{\Theta}=\mathrm{im}\,d^1$, $H^2_{\Theta}=Z^2_{\Theta}/B^2_{\Theta}$
**検証状態**: 一般形は未証明。現在は両極の probe・selector 条件面・その CPS 内在化条件・weak-package test・counterexample surface・guard-probe surface が揃っている。`calculations/計算_QM_H2Theta最小インスタンス.md` では `V = \mathbb{R}^2` 上の交代双線形 2-cocycle $\omega(u,v)=\tfrac12(xp' - px')$ が `d^2\omega=0` かつ `\omega \notin \mathrm{im}\,d^1` を満たすことを固定し、QM を nontrivial candidate とした。`calculations/計算_FTC_H2Theta対照例.md` では可縮な区間上の FTC 最小模型で forgetting law `Θ` が global potential 由来の exact 1-cochain となり、obstruction class `[ω_{\Theta}]` が立たないことを固定した。`calculations/計算_H2Theta_selector条件面.md` では `global potential / contractible patching` と `alternating witness / central extension` を selector の十分条件の二極として分離した。`calculations/計算_H2Theta_selector_CPS内在化条件.md` ではこれらを `CPS-globalization atlas + zero holonomy` および `CPS-obstruction witness + irreducible orientation` へ翻訳した。`calculations/計算_H2Theta_selector_package弱化テスト.md` では full package を `support-local re-lift + generator-face zero holonomy` と `support-local faithful witness + anti/symmetric separation` まで削った。`calculations/計算_H2Theta_selector_weakpackage反例探索.md` では `Weak Lift A' / B'` の false trivialization / false obstruction を分け、guard 付き版 `A'' / B''` の必要性を固定した。さらに FTC/QM の既存 note へ guard probe を追加し、FTC では `A''`、QM では `B''` が local model / minimal model で自然に通ることを確認した。2026-04-17 時点で front-stage pair は `貼合側 / 残差側` に固定し、`A'' / B''` は backstage の guarded package 名として保持する。したがって C4 は少なくとも **selector surface / lift surface / weak-package surface / counterexample surface / guard-probe surface** を持つ canonical conjecture になった。ただしこれはなお Paper II canonical surface の probe 水準であり、本文主張を partial theorem へ昇格させるものではない
...
4. 論文 XIV (曲率は忘却の繰り上がりである) が同じ方向で既に独立に進行中 (§M7 Phase 5 blocker 2 件目)。C4 はこの接続点でもある
...
**決定 1 (主): A'' / B'' を本文語へ圧縮するか**
- 選択肢 α: guard probe が通ったので `§3.4.6` に 1-2 行の補注として `Weak Lift A'' / B''` の guarded name だけを入れる
- 選択肢 β: calculations + meta に留め、本文は "C4 は conjectural surface" の記述のみ維持する (現状維持)
- 選択肢 γ: guard 各語 (`support-saturated`, `coboundary-conservative`, `refinement-stable` 等) を 統一記号表 or 補助 note に登録してから §3.4.6 に入れる
- エージェント推奨: **β 優先** (現時点では proof なし conjecture の水準は変化なし; 本文侵入は必要十分検証後が強い)
...
- v0.2 (2026-04-14): C4 (Coherence Defect Lemma) cochain-first 探索枝を追加。§M1 に次段拡張 remark / §M2 に C4 新設 / §M5 に C4 Round 1-3 全走行 (全て射程維持 ✓) / §M7 Phase 5 blocker に C4 形式化試行追加。Codex (2026-04-14) の「埋まらない殻」補正と Coherence Defect Lemma 命名を採用。計算ノート `Face補題_ホモロジー厳密化.md` §1 H_1 却下論理との射程分離を chain degree + homology/cohomology 方向の二重差分で明示。±3σ 入口ゲート通過。
...
- v0.13 (2026-04-16): `/bou.momentum+ Round 5 accelerate` で §M7 donor 統合欄の欠落補完。v0.9 統合済みの `計算_H2Theta_selector_CPS内在化条件.md` (lift surface)、v0.10 統合済みの `計算_H2Theta_selector_package弱化テスト.md` (weak-package surface)、v0.11–v0.12 統合済みの `計算_H2Theta_selector_weakpackage反例探索.md` (counterexample + guard-probe surface) の 3 donor エントリを §M7 に追記した。QM / FTC の既存エントリに v0.12 guard probe 追記を同期した。§M7 末尾に著者決定パッケージ (決定 1: A''/B'' 本文圧縮判断 / 決定 2: UCT bridge C5 / 決定 3: 符号理論三分法) を新設した。
- v0.14 (2026-04-17): `/ene` で front-stage / backstage の二層語彙を実装。本文 `§3.4.6` に `貼合側 / 残差側` を導入し、`H^2_{\Theta}=0` を局所 law が一つの global law にまとまる側、`[\omega_{\Theta}] \neq 0` を局所整合の残差 class が残る側として圧縮した。`drafts/infra/FaceLemma_符号理論対応.md` では `FTC = 貼合側`, `QM = 残差側` を front-stage 対応として固定し、`A'' / B''` は backstage の guarded package 名として保持する方針を明示した。対応本文は v1.16。
