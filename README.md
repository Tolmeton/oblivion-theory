...
- `drafts/standalone/トークン_最適化器_エムシーピー_忘却随伴メモ.md` は
  `token-optimizer-mcp` を可逆忘却の局所実装として読むための
  companion memo。
- 現行の衛星クラスタ:
...
- Face Lemma の概念入口は `drafts/infra/リファレンス/FaceLemma.md`。符号理論対応・修復可能性設計・calculations
  符号理論対応・修復可能性設計・calculations 進捗を含む backstage は
  `drafts/infra/FaceLemma_技術設計.md`。
- `drafts/infra/リファレンス/モノグラフ構成設計.md` は
...
- `drafts/incubator/`
- active workbench は `面五補題_草稿.md`,
  `文書_0082.md`, `文書_0084.md` の 3 本に固定する。
- `legacy/` は公開レイヤから外した履歴置き場であり、
  現行の保持対象は `legacy/力とは忘却である_v2.md`。
- それ以外の橋渡しメモ、先行研究超克、AI接続メモは
  `standalone/`, `infra/`, `infra/リファレンス/` に再配置する。

## 補助作業面

- `calculations/` は計算ノート、証明パッチ、数値実験レポートを置く補助面。
- `experiments/` は実験設計・実験用チェックリストの補助面。
- `drafts/infra/リファレンス/熱力学対応表.md` は熱力学対応表のリビングノートであり、
  4レイヤの入口には数えない。
...
- 旧 `drafts/infra/*.md` 参照が残っている場合、正本 alias は
  `drafts/infra/リファレンス/忘却論オンボーディング.md` の
  legacy 対応表を参照する。
