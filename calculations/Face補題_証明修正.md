...
**符号理論的読み.** (S2) の「非退化 2-simplex を含む」は、そのまま圏論版 syndrome 条件と読める。すなわち CPS-安定性は persistence ではなく、欠損が defect として露出しうる最小検査面の存在を要求している。stable / detectable / recoverable の三分法は `drafts/incubator/FaceLemma_技術設計.md` に整理した。
